...
init(Reader, header_only) ->
    {ok,mp4:open(Reader, header_only)};
...
start() ->
    {ok, Dev} = file:open("test3gp.3gp",[read,binary]),
    Reader = {file,Dev},
    {ok,Media} = init(Reader,header_only),
    %%io:format("~p~n", [media_info(Media)]),
    %%io:format("~p~n", [keyframes(Media)]),
    %%Data = read_frame(Media,undefined),
    %%%%Data = read_frame(Media,#frame_id{id = 1,v = video}),
    %%[Track1|_] = tuple_to_list(Media#mp4_media.tracks),
    %%io:format("duration ~p~n", [Track1#mp4_track.duration]),
    %%io:format("chunk_sizes ~p~n", [Track1#mp4_track.chunk_sizes]),
    %%io:format("sample_sizes ~p~n", [Track1#mp4_track.sample_sizes]),
    %%io:format("~p~n",[size(Data#video_frame.body)]),
    [Video|[_]] = mp4:read_media_frames(Media#mp4_media{reader = Reader}),
    lists:map(fun(Frame) -> 
                <<PSC:22,TR:8,PTYPE1:5,PTYPE2:3,Bit9:1,_/bitstring>> = Frame, 
                io:format("PSC: ~p TR: ~p PTYPE1: ~p PTYPE2: ~p Bit9:~p~n",[PSC,TR,PTYPE1,PTYPE2,Bit9]) end,
                Video).
