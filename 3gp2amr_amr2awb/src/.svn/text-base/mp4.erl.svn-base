...
%
%open(Reader, header_only) ->
%    io:format("open header only~n"),
%    {ok, Media} = read_header(Reader),
%    Media;
%
%open(Reader, Options) ->
...
%  end.
%
%read_data({M, Dev} = _Reader, Offset, Size) ->
...
%  MediaInfo#mp4_media{width = Width, height = Height, tracks = [Track|Tracks]};
%
%append_track(#mp4_media{tracks = Tracks} = MediaInfo, #mp4_track{} = Track) ->
%  MediaInfo#mp4_media{tracks = [Track|Tracks]}.
%
%
...
%   ?D({"Timescale: ", TimeScale, "duration: ",Duration, extract_language(Language)}),
...
%
%samr(<<_Reserved:2/binary, _RefIndex:16/binary, _Unknow:10/binary, Atom/binary>> = AMR, Mp4Track) ->
%  ?D(AMR),
...
%  io:format("stsz simple count: ~p~n",[SampleCount]),
%  [io:format("stsz sample size: ~p~n",[Size]) || <<Size:32>> <= SampleSizeData],
%  Mp4Track#mp4_track{sample_sizes = [
%          Size
%          || <<Size:32>> <= SampleSizeData]};
%
%stsz(<<_Version:8, _Flags:24, Size:32, SampleCount:32>>, Mp4Track) ->
%    Sizes = [Size || _ <- lists:seq(1,SampleCount)],
%  Mp4Track#mp4_track{sample_sizes = Sizes}.
%
...
%  [io:format("stts count: ~p duration: ~p~n",[Count, Duration]) ||
%                                <<Count:32, Duration:32>> <= Rest],
...
%    io:format("stsc entry count: ~p~n", [EntryCount]),
%    [io:format("first chunk: ~p~nsamples per chunk ~p~nsample descrip idx ~p~n",
%                [F,S,Sd]) || <<F:32, S:32, Sd:32>> <= Rest],
...
%%%
%stco(<<0:8, _Flags:3/binary, OffsetCount:32, Offsets/binary>> = STCO, Mp4Track) ->
%  size(Offsets) == OffsetCount*4 orelse erlang:error({invalid_stco,OffsetCount,size(Offsets)}),
%  io:format("stco chunk : ~p~n",[STCO]),
%  [io:format("stco chunk offset: ~p~n",[Offset]) || <<Offset:32>> <= Offsets],
...
%    %%{Frames, IndexInfo} = fill_track(TrackId, SampleSizes, [1], [1], Keyframes, Timestamps, Compositions, Timescale),
%
...
%
%
%read_media_frames(#mp4_media{reader = Reader, tracks = Tracks} = _Media) ->
%    io:format("reader is: ~p~n",[Reader]),
%    lists:map(fun(Track) ->
%                read_track_frames(Reader,Track) end,
%              Tracks).
%read_track_frames(Reader, #mp4_track{sample_sizes = Sample_sizes, chunk_offsets = Chunk_offsets} = _Track) ->
%    io:format("sample ~p~n chunk ~p~n",[length(Sample_sizes), length(Chunk_offsets)]),
%    %%Frame_metas = lists:zip(Chunk_offsets, Sample_sizes),
%
%    %%debug me! here is for one chunk has 301 samples
%    [Start|_] = Chunk_offsets,
%    %%io:format("Start is ~p~n",[Start]),
%    Noffsets = sum_seq(Start,Sample_sizes,[Start]),
%    %%io:format("~p~p~n",[length(Noffsets),Noffsets]),
%    Frame_metas = lists:zip(Noffsets, Sample_sizes),
%    lists:map(fun({Offset, Size}) -> {ok,Data} = read_data(Reader, Offset, Size), Data end, Frame_metas).
%
%sum_seq(Acc, [H|T], Result) ->
%    sum_seq(Acc + H, T, [Acc+H|Result]);
%sum_seq(_,[],Result) ->
%    [_|T] = Result,
%    lists:reverse(T).
%
