...
/*-------------------------------------------------------------------*
* write_indices_to_buffer()
*
* Write the buffer of indices to a file
*-------------------------------------------------------------------*/

void write_indices_to_buffer(
    Encoder_State *st,  /* i/o: encoder state structure                                     */
    UWord8 *buffer,         /* i  : output bitstream buffer                                      */
    UWord8 *pFrame,     /* i: byte array with bit packet and byte aligned coded speech data */
    Word16 pFrame_size  /* i: size of the binary encoded access unit [bits]                 */
)
{
    ...
    buffer[0] = header;
    /* Write speech bits */
    memcpy(&buffer[1], pFrame, (pFrame_size + 7)>>3);
...
{
    Word16 isAMRWB_IOmode, core_mode = -1;
    Word32 total_brate;
    Word16 num_bits;
    if (feof(f_stream)) {
        return 0;
    }
    if (!fread(header, sizeof(UWord8), 1, f_stream)) {
        return 0;
    }
    ...
    */
    ...
    if (!fread(frame_body, sizeof(UWord8), (num_bits + 7)>>3, f_stream)) {
        return 0;
    }
    return 1;
}
...
)
{
    ...
    */
    ...
    if(  ( st->bfi != 0 && st->ini_frame == 0) &&    /*  ini_frame can not be used when rewflag is 1  */
