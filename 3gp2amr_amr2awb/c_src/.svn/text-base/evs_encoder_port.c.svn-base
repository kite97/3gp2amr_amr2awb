...
int main( int argc, char** argv )
{
    ...
    //FILE* state_stream = fopen("state0", "wb");
    //fwrite(st, 1, sizeof(Encoder_State), state_stream);
    //fclose(state_stream);
    //return 0;
    ...
    while( (n_samples = (short)fread(data, sizeof(short), input_frame, f_input)) > 0 )
    {
        ...
        //               , pFrame, pFrame_size
        //             );
        write_indices_to_buffer( st, buffer
                       , pFrame, pFrame_size
                     );
        fwrite(buffer, 1,  ((pFrame_size + 7)>>3) +1, f_stream);
        fflush(stdout);
